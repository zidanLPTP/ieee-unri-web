...
export default function AboutPage() {
  ...
  const dynamicLeaders = [
    ...
    getOfficerByRole("Vice Director I", "Academic & Research"),
    getOfficerByRole("Vice Director II", "Human Resources"),
    getOfficerByRole("Vice Director III", "Project & Development"),
  ];
  ...
  const dynamicDivisions = staticDivisions.map(div => {
     const coordinator = officers.find(o => o.division === div.name && o.position === "Head of Division");
     return {
        ...div,
        coordinator: coordinator ? coordinator.name : "TBA"
     };
  });
  ...
  const LeaderCard = ({ item, delay = 0 }: { item: any, delay?: number }) => (
    <motion.div 
      ...
      viewport={{ once: true }}
      transition={{ delay }}
      className="group relative h-[420px] w-full rounded-[2rem] overflow-hidden bg-[#151b2b] border border-white/5 hover:border-[#E7B95A]/50 transition-all duration-500 shadow-xl"
    >
  ...
  return (
    <main className="min-h-screen bg-[#0C101C] text-white overflow-hidden selection:bg-[#E7B95A] selection:text-[#0C101C]">
      <Navbar />
      ...
      <section className="py-20 px-6 container mx-auto relative z-10">
        <div className="grid lg:grid-cols-12 gap-6">
          ...
          <motion.div 
            ...
            viewport={{ once: true }}
            className="lg:col-span-7 bg-[#151b2b] border border-white/5 p-10 rounded-[2.5rem] relative overflow-hidden group hover:border-[#3386B7]/30 transition-all"
          >
          ...
          <motion.div 
             ...
             className="lg:col-span-12 bg-[#151b2b] border border-white/5 p-10 md:p-14 rounded-[2.5rem] mt-4"
          >
             ...
             <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
               {visionMission.missions.map((mission, idx) => (
                 <div key={idx} className="flex gap-4 items-start">
                   <div className="w-8 h-8 rounded-full bg-[#3386B7]/20 flex items-center justify-center text-[#3386B7] font-bold text-sm shrink-0 mt-1">
                     {idx + 1}
                   </div>
      ...
      <section className="py-24 px-6 bg-[#0C101C] relative">
         <div className="absolute top-[20%] left-1/2 -translate-x-1/2 w-[800px] h-[800px] bg-[#3386B7]/5 rounded-full blur-[120px] pointer-events-none" />
      ...
      <section className="py-20 px-6 container mx-auto pb-32 relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
           ...
           {staticDivisions.map((div, idx) => {
             const coordinator = officers.find(o => o.division === div.name && o.position === "Head of Division");
             const coordName = coordinator ? coordinator.name : div.coordinator;
             ...
             return (
               <Link href={`/departments?tab=${div.id}`} key={idx} className="block h-full">
                 <motion.div 
                   ...
                   className="group h-full rounded-[2.5rem] bg-[#151b2b] border border-white/5 hover:border-[#3386B7]/50 transition-all duration-300 hover:shadow-[0_0_30px_rgba(51,134,183,0.15)] flex flex-col overflow-hidden relative"
                 >
