...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function initData() {
        ...
        if (res.success && res.stats && res.data) {
            ...
            setEvents(res.data.events.map(e => ({ ...e, date: fmtDate(e.date) })));
            setNews(res.data.news.map(n => ({ ...n, date: fmtDate(n.date) })));
            setGallery(res.data.gallery.map(g => ({ ...g, date: fmtDate(g.date) })));
        }
        setLoading(false);
    }

    const storedUser = localStorage.getItem('adminUser') || '{"name":"Admin","role":"Staff"}';
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
        if (activeTab === 'events') {
            ...
            if (result.success) setEvents(prev => prev.filter(item => item.id !== id));
        } else if (activeTab === 'news') {
            result = await deleteNews(id);
            if (result.success) setNews(prev => prev.filter(item => item.id !== id));
        } else if (activeTab === 'gallery') {
            result = await deleteGallery(id);
            if (result.success) setGallery(prev => prev.filter(item => item.id !== id));
        }
  ...
  const displayedItems = currentData.slice(startIndex, startIndex + itemsPerPage);
    ...
    <div className="min-h-screen bg-[#0C101C] text-white flex selection:bg-[#E7B95A] selection:text-[#0C101C] relative">
      ...
      <main className="flex-1 p-4 md:p-10 overflow-y-auto w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
           <StatCard title="Total Personnel" value={stats.totalMembers} subtitle="Registered Members" icon={Users} color="text-blue-400" bg="bg-blue-500/10" />
           <StatCard title="Departments" value={stats.totalDivisions} subtitle="Operational divisions" icon={LayoutDashboard} color="text-purple-400" bg="bg-purple-500/10" />
           <StatCard title="Total Content" value={stats.totalContent} subtitle="Events, News & Photos" icon={FileText} color="text-[#E7B95A]" bg="bg-[#E7B95A]/10" />
        </div>
        ...
        <div className="bg-[#151b2b] border border-white/5 rounded-3xl overflow-hidden flex flex-col min-h-[500px]">
           ...
           <div className="flex-1 p-4 md:p-6 overflow-x-auto">
              {loading ? (
                 <div className="h-full flex flex-col items-center justify-center py-12">
                    ...
                    <p className="text-gray-500 text-xs">Loading data from database...</p>
                 </div>
                 ...
                 <div className="space-y-2 min-w-[600px] md:min-w-0">
                    ...
                    {displayedItems.map((item) => (
                       <div key={item.id} className="grid grid-cols-12 gap-4 px-4 py-4 bg-[#0C101C] border border-white/5 rounded-xl items-center hover:border-[#E7B95A]/30 transition-all group">
                          <div className="col-span-6 md:col-span-5 flex items-center gap-3">
                             <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${activeTab === 'events' ? 'bg-blue-500/10 text-blue-400' : activeTab === 'news' ? 'bg-purple-500/10 text-purple-400' : 'bg-green-500/10 text-green-400'}`}>
                          ...
                          <div className="col-span-3 text-xs text-gray-400">{item.date}</div>
                          <div className="col-span-3 text-xs text-gray-400">{item.author || "Admin"}</div>
           ...
           {currentData.length > 0 && (
             <div className="p-4 border-t border-white/5 flex justify-between items-center bg-[#0C101C]/30">
                <p className="text-xs text-gray-500">Showing <b>{startIndex + 1}</b> - <b>{Math.min(startIndex + itemsPerPage, currentData.length)}</b> of <b>{currentData.length}</b></p>
                <div className="flex gap-2">
                   <button onClick={handlePrevPage} disabled={currentPage === 1} className={`p-2 rounded-lg border border-white/10 transition-colors ${currentPage === 1 ? 'text-gray-700 cursor-not-allowed' : 'text-white hover:bg-white/10'}`}>
                      <ChevronLeft size={16} />
                   </button>
                   <button onClick={handleNextPage} disabled={currentPage >= totalPages} className={`p-2 rounded-lg border border-white/10 transition-colors ${currentPage >= totalPages ? 'text-gray-700 cursor-not-allowed' : 'text-white hover:bg-white/10'}`}>
